...
const THEMES = {
  default: {
    ...
    music      : 'audio/classic-meadow.wav',
  },
  ...
  beach: {
    ...
    music      : 'audio/the-beach-tropical.wav',
  },
...
//  BACKGROUND MUSIC
// ─────────────────────────────────────────────
// One shared <audio> element, swapped between theme tracks. `loop` handles
// "keep circulating the track regardless of its length" natively — no
// manual replay logic needed. Autoplay only works following a user
// gesture, so this is only ever kicked off from click handlers (the start
// button and the theme buttons), never on page load.
let musicMuted = false;
try { musicMuted = localStorage.getItem('pinguHopMuted') === '1'; } catch (e) { /* ignore */ }
...
function ensureThemeMusic() {
  const src = activeTheme.music;
  if (!src) { bgMusic.pause(); return; }
  // Only reset/restart the track when it's actually a different theme's
  // music — switching to the theme you're already on (e.g. restarting the
  // same run) shouldn't jolt the track back to 0:00.
  const resolvedSrc = new URL(src, document.baseURI).href;
  if (bgMusic.src !== resolvedSrc) {
    bgMusic.src = src;
  }
  bgMusic.muted = musicMuted;
  bgMusic.play().catch(() => { /* blocked until a user gesture; harmless */ });
}

function setMusicMuted(muted) {
  musicMuted = muted;
  bgMusic.muted = muted;
  try { localStorage.setItem('pinguHopMuted', muted ? '1' : '0'); } catch (e) { /* ignore */ }
  const btn = document.getElementById('mute-btn');
  if (btn) btn.textContent = muted ? '🔇' : '🔊';
}

// ── Sound effects ──────────────────────────────
// Short one-shot clips. Each play clones the base node instead of reusing
// one Audio element, so overlapping triggers — rapid hops, or a coin
// grabbed while the last pickup sound is still finishing — layer on top of
// each other instead of cutting each other off. They share the same mute
// toggle as the music.
const hopSfx      = new Audio('audio/penguin-hop.wav');
const coinSfx     = new Audio('audio/coin-collect.wav');
const gameOverSfx = new Audio('audio/penguin-game-over.wav');

function playSfx(base, volume) {
